...
export function MobileWrapper({ children }: MobileWrapperProps) {
  useEffect(() => {
    // Prevent zoom on iOS (only if not in development)
    ...
    // Only add touch event listeners if not in development environment
    const isDev = window.location.hostname.includes('figma') || 
